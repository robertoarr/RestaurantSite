    ...
    stages {
        stage('Build') {
            steps {
                ...
                sh 'docker run -d -p 6379:6379 --name redis_test_cont redis:2.8'
                sh 'if [ "$(docker ps -aq -f name=mysql_test_cont)" ]; then docker rm -f mysql_test_cont; fi'
                sh 'docker run -d -e MYSQL_ROOT_PASSWORD=root -p 3308:3306 -v /home/roberto/Projects/jenkins/start_db.sql:/start_db.sql --name mysql_test_cont mysql/mysql-server:8.0.11'
