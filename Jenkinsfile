    ...
    stages {
        stage('Build') {
            steps {
                ...
                sh 'docker run -d -e MYSQL_ROOT_PASSWORD=root -p 3309:3306 -v $JENKINS_DIR/start_db.sql:/start_db.sql --name mysql_test_cont mysql/mysql-server:8.0.11'
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'docker run -d -e MYSQL_ROOT_PASSWORD=root -p 3308:3306 -v $JENKINS_DIR/start_db.sql:/start_db.sql --name mysql_develop_cont mysql/mysql-server:8.0.11'
                ...
                sh "docker run -d -t -e PYTHONUNBUFFERED=0 -p 8000:8000 -v $JENKINS_DIR/.env_develop:/code/.env --name api_develop_cont api_image:${env.BUILD_NUMBER} python manage.py runserver 0:8000"
