    ...
    stages {
        stage('Build') {
            steps {
                ...
                sh 'docker run -d -e MYSQL_ROOT_PASSWORD=root -p 3309:3306 -v /restautant_develop_util/start_db.sql:/start_db.sql --name mysql_test_cont mysql/mysql-server:8.0.11'
        ...
        stage('Test'){
            steps {
                sh "docker run --rm -v /restautant_develop_util/.env_tests:/code/.env api_image:${env.BUILD_NUMBER} python manage.py test --noinput -k"
            }
            post {
               always {
                    sh 'if [ "$(docker ps -aq -f name=redis_test_cont)" ]; then docker rm -f redis_test_cont; fi'
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'docker run -d -e MYSQL_ROOT_PASSWORD=root -p 3308:3306 -v /restautant_develop_util/start_db.sql:/start_db.sql --name mysql_develop_cont mysql/mysql-server:8.0.11'
                ...
                sh "docker run -d -t -e PYTHONUNBUFFERED=0 -p 8000:8000 -v /restautant_develop_util/.env_develop:/code/.env --name api_develop_cont api_image:${env.BUILD_NUMBER} python manage.py runserver 0:8000"
